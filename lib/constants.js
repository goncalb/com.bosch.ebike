...
// Theft-detection service (ConnectModule last known location; requires
// eBike Alarm feature). Unofficial v0 API - may change without notice.
const THEFT_DETECTION_BASE_URL = 'https://theft-detection.prod.connected-biking.cloud';

// Rider-activity service (ride history). Same cloud + token as the profile API.
const ACTIVITY_BASE_URL = 'https://obc-rider-activity.prod.connected-biking.cloud';
...
const ENDPOINTS = {
  ...
  PROFILE:         '/v1/profile',
  LATEST_LOCATIONS: (bikeId) => `/v0/latest-locations?bikeId=${encodeURIComponent(bikeId)}`,
  ACTIVITIES:       (size = 1) => `/v1/activity?page=0&size=${size}&sort=-startTime`,
};
...
const CAP = {
  ...
  LAST_LOCATION:     'ebike_last_location',
